(function () {
  ...
  function trackClick(identifier) {
    ...
    fetch(`${API_HOST}/track/click`, {
      ...
      .then((data) => {
        if (data) {
          cookieManager.set(PIMMS_ID_VAR, data.clickId);
        }
      });
  }
  ...
  function init() {
    ...
    const shouldSetCookie = () => {
      return (
        !cookieManager.get(PIMMS_ID_VAR) || ATTRIBUTION_MODEL !== 'first-click'
      );
    };

    // Direct click ID in URL
    const clickId = params.get(PIMMS_ID_VAR);
    if (clickId && shouldSetCookie()) {
      cookieManager.set(PIMMS_ID_VAR, clickId);
      return;
    }
  ...
  window._pimmsAnalytics = {
    ...
    i: PIMMS_ID_VAR, // was PIMMS_ID_VAR
